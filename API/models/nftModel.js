...
//Virtual Property is that property which is not stored in database but it is attached to the item which can be seen in log
...
//DOCUMNT MIDDLEWARE: runs before .save() or .create() and not at the time of update
...
nftSchema.post(/^find/, function (doc, next) {
  console.log(`Query took time: ${Date.now() - this.start} ms`);
  // console.log(doc);
  next();
});
